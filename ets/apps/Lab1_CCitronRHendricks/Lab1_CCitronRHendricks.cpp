/*****************************************************************************
*                                                                            *
*       Source title:   Lab1_CCitronRHendricks.CPP
...
#include "config.h"
#include "testmain.h"
#include "Lab1_CCitronRHendricks.h"
...
// file.
//
CLab1_CCitronRHendricks theApp(ProductDatasheetFlow);
...
BOOL CLab1_CCitronRHendricks::UserInit( void )
{
    //////////////////////////////////////////////////////////////////////////
    ...
    //
#if 1
    SetGlobalSwitchName( GP1, "Measure Voltage" );
    SetGlobalSwitchName( GP2, "Measure Current" );

	SetTestMainVariable(GP1, TRUE);
	SetTestMainVariable(GP2, TRUE);

	SetTestMainVariable(STOP_ON_FAIL, TRUE);
#endif
...
BOOL CLab1_CCitronRHendricks::UserExit( void )
{
	apuset(resistor, APU_FV, 0, APU_10V, APU_10MA, PIN_TO_VI);
	apuset(resistor, APU_VIOFF, 0, APU_10V, APU_10MA, PIN_TO_VI);
    return TRUE;
}
...
BOOL CLab1_CCitronRHendricks::OnTestInit( void )
{
    //////////////////////////////////////////////////////////////////////////
...
BOOL CLab1_CCitronRHendricks::OnTestExit( void )
{
	apuset(resistor, APU_FV, 0, APU_10V, APU_10MA, PIN_TO_VI);
	apuset(resistor, APU_VIOFF, 0, APU_10V, APU_10MA, PIN_TO_VI);
    return TRUE;
...
BOOL CLab1_CCitronRHendricks::OnTestStartup( void )
{
...
BOOL CLab1_CCitronRHendricks::OnTestCompletion( void )
{
	apuset(resistor, APU_FV, 0, APU_10V, APU_10MA, PIN_TO_VI);
	apuset(resistor, APU_VIOFF, 0, APU_10V, APU_10MA, PIN_TO_VI);

	// Enable operator box lights
	int binNum = msBinValue(MS_SITE0);
	bin(binNum - 2, (binNum - 1) > 0 ? 1: 0);
	aux(binNum == 2 ? -1 : 0);
...
BOOL CLab1_CCitronRHendricks::OnLogData( void )
{
    int PassFail;
...
BOOL CLab1_CCitronRHendricks::OnFailSite( int site )
{
	apuset(resistor, APU_FV, 0, APU_10V, APU_10MA, PIN_TO_VI);
	apuset(resistor, APU_VIOFF, 0, APU_10V, APU_10MA, PIN_TO_VI);
    return TRUE;
...
int CLab1_CCitronRHendricks::OnSot(int SotValue)
{
    return SotValue;
}
...
BOOL CLab1_CCitronRHendricks::OnUtilError(int errnum)
{
    return TRUE;
}
...
BOOL CLab1_CCitronRHendricks::OnInterlock(ULONG intlocks)
{
    return TRUE;
}
...
// Test Function: forceVoltage
ETS_PRGFLOW_FUNC forceVoltage( int DSIndex, LPCTSTR TestLabel )
{
	// User switch and datasheet row
	if (!GetUserSwitch(GP1)) {
	...
	// Set test conditions
	apuset(resistor, APU_FV, 0, APU_10V, APU_10MA, PIN_TO_VI);
	apuset(resistor, APU_FV, 5, APU_10V, APU_10MA, PIN_TO_VI);
	lwait(10);

	// Measure results and log
	double voltage = apumv(resistor, 100, 13);
	double current = apumv(resistor, 100, 13);
	double resistance = voltage / (current / 1000);

	msLogResult(MS_SITE0, DSIndex++, voltage);
	msLogResult(MS_SITE0, DSIndex++, current);
...
// Test Function: forceCurrent
ETS_PRGFLOW_FUNC forceCurrent( int DSIndex, LPCTSTR TestLabel )
{
	// User switch and datasheet row
	...
	// Set test conditions
	apuset(resistor, APU_FI, 0, APU_10V, APU_10MA, PIN_TO_VI);
	apuset(resistor, APU_FI, 5, APU_10V, APU_10MA, PIN_TO_VI);
	lwait(10);
